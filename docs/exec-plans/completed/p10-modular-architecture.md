...
- [x] `ModuleRegistry` 能注册和列举模块
- [x] 每个模块独立调用 `check_capabilities()` 检测可用性
- [x] `/api/capabilities` 返回所有模块状态
- [x] 缺少依赖时模块标记为 `disabled`，系统仍然正常启动
...
- [x] 无 `LINEAR_API_KEY` 时 Linear 模块标记为 `disabled`
- [x] 无 Linear 时本地任务跟踪自动启用
- [x] 前端 board 在两种模式下正常运行
- [x] 有 Linear 时功能和现在完全一致（回归测试）
...
- [x] 三个 Agent 后端都是独立模块（claude/codex/deepseek）
- [x] 只在一个模块可用时，dashboard 不会显示其他 module 的空面板
- [ ] ~~`/api/windows` 自动映射到已加载的 agent 模块~~ → **长期计划**。`usage_reader.py` 保持独立，各后端 reader 统一接口待 P12 测试覆盖时处理
- [x] 切换 agent 时只显示该 agent 的数据
...
- [x] STT 模块在后端注册，检测 `OPENAI_API_KEY`
- [x] 无 key 时前端 STT 选择下拉不显示 "OpenAI" 选项（或显示并 disable）
- [x] 浏览器 STT 始终可用（不需要后端）
- [x] `POST /api/stt` 在无 key 时返回 400 带可读错误信息
- [ ] ~~WhisperCPP 本地 STT~~ → **长期计划**。需要本地 whisper 模型文件，无明确需求前搁置
...
- [x] 前端在启动时 fetch `/api/capabilities`
- [x] 所有动态 UI 元素根据 capabilities 显示/隐藏（agent/STT/tracker 切换）
- [x] 没有 JavaScript 错误因为缺失的 API 端点
...
- [x] Phase 6a: Linear 路由 → `runner/modules/linear/`（-270 行）
- [x] Phase 6b: `_linear_polling_loop` → `runner/modules/linear/`；`_stall_detection_loop` → `runner/modules/stall_detection/`
- [x] Phase 6c: PTY 路由 + SocketIO 事件 → `runner/modules/pty/`
- [x] Phase 6d: 死 Linear 代码清理（-294 行）
- [x] Phase 6f: Registry 函数 → `runner/core/registry_loader.py`
- [ ] ~~`app.py` 减少到 500 行以内~~ → **长期计划**。核心 Dispatch Engine（run dispatch/retry/_ai_command）占 ~1,300 行，函数间耦合紧密，强行拆分需注入大量 app-level 状态，收益递减。当前 1,849 行（-31%）已是实用平衡点
- [x] 所有功能正常工作（回归测试通过）
- [x] 新模块可以在 `runner/modules/` 下编写并通过注册表自动发现
...
- [x] `/api/capabilities` 返回的 `backends` 列表可供 Symphony 动态读取（P10 已交付）
- [ ] ~~WORKFLOW.md 中的 `agent.backend` 修改后，Symphony 能感知到变化~~ → **长期计划**（P11）。需 Symphony 端实现文件 watch 或定重读
- [ ] ~~Symphony 通过 agentic-os 的 `POST /run` dispatch，不直接调用 agent CLI~~ → **长期计划**（P11）。需修改 symphony dispatch 逻辑
- [ ] ~~Symphony 展示不可用的 Agent 后端时显示禁用状态~~ → **长期计划**（P11）。同前端 dashboard 模式
...
| 组件 | 行数 |
|------|------|
| `runner/app.py`（缩减后） | 1,849 |
| `runner/core/module_registry.py` | 261 |
| `runner/core/features.py` | 76 |
| `runner/core/registry_loader.py` | 130 |
| `runner/modules/linear/__init__.py` | 568 |
| `runner/modules/linear/local_tracker.py` | 233 |
| `runner/modules/backends/claude.py` | 76 |
| `runner/modules/backends/codex.py` | 72 |
| `runner/modules/backends/deepseek.py` | 61 |
| `runner/modules/stt/__init__.py` | 105 |
| `runner/modules/pty/__init__.py` | 232 |
| `runner/modules/stall_detection/__init__.py` | 75 |
| **总提取代码量** | **3,738** |
...
|------|------|------|
| 基线 | P10 开始 | 2,681 |
| Phase 6a | Linear 路由移出 | -270 |
| Phase 6b | 轮询迁移 | ~0 |
| Phase 6c | PTY + Stall 提取 | -29 |
| Phase 6d | 死 Linear 代码 | -294 |
| Phase 6f | Registry 提取 | -62 |
| **最终** | **P10 结束** | **1,849 (-31%)** |
...
| Commit | Phase |
|--------|-------|
| `P10: modular architecture foundation + Phase 1 module registry` | Phase 1 |
| `P10 Phase 2: Linear modularization + LocalTracker fallback` | Phase 2 |
| `fix: remove duplicate .env loading in LinearModule module init` | 修复 |
| `P10 Phase 3: Agent backend modularization + Phase 6a Linear route extraction` | Phase 3 + 6a |
| `P10 Phase 4: STT multi-backend module` | Phase 4 |
| `Phase 6c: StallDetectionModule + app.py thinning` | Phase 6c |
| `Phase 6d: Remove dead Linear code from app.py` | Phase 6d |
| `Phase 6f: RegistryLoader extraction` | Phase 6f |
